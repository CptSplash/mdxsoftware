...
const TabsList = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('inline-flex h-10 items-center justify-start rounded-md bg-gray-100 p-1 text-gray-600', className)}
      {...props}
    />
  )
)
    ...
    return (
      <button
        ...
        className={cn(
          ...
          isActive ? 'bg-white text-[#1E3A5F] shadow-sm font-semibold' : 'text-gray-600 hover:text-gray-900',
          className
        )}
        {...props}
      />
